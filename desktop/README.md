...
首次安装（或补建入口）由安装 Agent 自动完成：先读取上层安装目录的 `desktop/client-release.json`，下载并解压匹配的已核验客户端包，再由固定版本应用执行：

```
AI Carry.exe --install-shortcuts --assistant-root "本次安装的 AI Carry 完整目录"
```

只创建系统实际桌面上的客户端入口；网页版保留在完整安装目录的 `dashboard.html`，客户端右上角的“网页版”按钮可以打开它。已有同一助手的客户端入口可更新；其他同名文件保留并另取名称。结果保存在应用用户目录的 `installation-result.json`。快捷方式失败只影响入口，不撤销安装或修改助手数据。不设置开机启动，不修改权限。已有旧版客户端时由安装 Agent 并排安装新版本，不覆盖旧目录，也不要求用户手动补装。
