...
- **同一台电脑换本地 Agent**：`host.prepare-agent-switch` 默认用 `dashboard/scripts/prepare-agent-switch.mjs` 在新目录建立独立副本，原目录保持不变，副本带有接手说明和独立 `copy_id`，后续不自动同步。用户也可以明确选择轮流共用原目录；这种方式没有实时同步或并发写保护，每次写入前必须重新读取受影响真源。只能接收文字时改用当前接入和任务所需的极小胶囊。新宿主只在副本中逐项重连本机绑定，不全局改写用户正文。
...
3. 宿主 B 返回：当前实例 ID 已确认；能按需读取和写回文件；无法保证离线提醒；自己的原生记忆里已有工作习惯，但只给出“资料整理习惯、近一年、主要来自用户对话”的概况，没有把全文倒进上下文。
4. AI Carry 为宿主 B 建一个很小的档案。用户今天要求整理项目资料，于是只加载与整理习惯和该项目有关的一条记忆、一项能力和验收标准。
