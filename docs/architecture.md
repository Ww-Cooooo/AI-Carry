...
带版本的条目记录当时设计，不是叠加到当前任务的永久规则。现行变更、升级与会话接续分别以 `COMPONENT_CHANGE.md`、`core/upgrade/UPGRADE-CONTRACT.md`、`UPGRADE_SESSION_ACTIVATION.md` 为准；默认原地保留实例自有内容、在同一会话接续，不做全量纳管或强制新建任务。
...
### AC-034：实例持久变化共用按需兼容协定，不全量纳管旧资源
...
升级只处理目标核心与确有需要的少量迁移；实例资产、工作区、本机层、私密层、未知内容和未命中的可选模块原地保留，不枚举正文、不复制或规范化。使用组件时才检查与当前任务有关的兼容问题，不能偷偷搬迁正在使用的内容；单项不兼容只隔离该能力。只有当前正式切换确实依赖且无法信任的身份、所有权或用户数据才暂停切换。不引入后台扫描、第二套审批或全产品回归。
