...
- 可选 `health`：只允许正式实例在日常 `operational` 投影隔离了无关坏项时出现。包含 `state="degraded"`、如实汇总的正安全整数 `isolated_item_count`、1–12 个稳定类别 ID 的 `affected_areas`、必须为 `true` 的 `source_data_preserved`，以及有界自然语言 `summary` 和 `next_step`。诊断明细最多输出 64 条，但不能因此阻断更多坏项的局部隔离。它不能出现在模板、演示或健康快照中，不能包含路径、正文、秘密或攻击载荷，也不能把当前动作目标的失败降级成告警。
...
- 可选 `accumulation`：积累地图的低敏只读投影，含 `records` 与 `links`。从已经读取并核验的资产来源、明确关联、结果验证记录和 Skill 导出来源提取；不增加正文扫描或第二套登记表。连线端点只引用本快照内的资产（`asset:`）、学习建议（`learning:`）、Skill（`skill:`／`export:`）或匿名记录（`record:`）。关系为 `source`、`learned`、`used`、`related`、`produced`；只有闭合的结果证据才能生成 `used` 并携带原 `success`／`failure`，不能把来源引用称为成功任务。匿名记录只有局部 ID 和 `task`／`source` 类别，不含原事件 ID、任务名称、正文、路径、宿主或私密引用。最多展示 256 个匿名记录及 1024 条关联，超出只设 `truncated=true`，不删除资产或中断其他页面。旧快照缺字段仍显示原条目；坏关联由地图局部略过并提示。地图不计入资产数量，不承担授权，也不补造缺失关系。
