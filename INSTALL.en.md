...
1. Read `desktop/client-release.json` and use the official package for the current platform and architecture (currently `v2.1.6`); never fetch a same-named file from another site. Downloading, unpacking and launching this package are part of the install, not a later user task.
