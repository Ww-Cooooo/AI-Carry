...
Download only the matching asset from the verified official v2.1.1 Release: `AI-Carry-2.1.1-win32-x64.zip`, `AI-Carry-2.1.1-darwin-arm64.zip`, or `AI-Carry-2.1.1-darwin-x64.zip`. Unpack to a new stable application folder. Do not overwrite an existing app or place user materials inside it. The desktop bundle includes its runtime; users need no npm installation or build.
